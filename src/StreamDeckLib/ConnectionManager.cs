	...
	public partial class ConnectionManager : IDisposable
	{
		                                           ...
		                                           IStreamDeckProxy streamDeckProxy = null)
		{
			using (var app = new CommandLineApplication())
			{
				...
				{
					throw new
						ArgumentException($"{nameof(commandLineArgs)} must be the commandline args that the StreamDeck application calls this program with.");
				}
			}
		}
				           ...
				           payload = new SetImageArgs.Payload
				                     {
					                     TargetType = SetTitleArgs.TargetType.HardwareAndSoftware,
					                     image =
						                     $"data:image/{new FileInfo(imageLocation).Extension.ToLowerInvariant().Substring(1)};base64, {imgString}"
				                     }
			           };
